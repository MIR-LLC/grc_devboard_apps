...
bool kws_model_init(const unsigned char *model_ptr, const char **labels,
                    size_t labels_num) {
  if (s_interpreter) {
  ...
  s_interpreter = new tflite::MicroInterpreter(model, s_micro_op_resolver,
...
static void dequantize(const TfLiteTensor *tensor, float *dst, size_t len) {
  for (int i = 0; i < len; i++) {
    dst[i] =
      (tensor->data.int8[i] - tensor->params.zero_point) * tensor->params.scale;
  }
}
...
int kws_recognize_word(const float *src, size_t len) {
  ...
  // set input
  set_input(src, s_interpreter->input(0), len);
  ...
  get_output(s_interpreter->output(0), out_buffer, s_labels_num);
  const size_t idx = argmax(out_buffer, s_labels_num);
  ESP_LOGI(__FUNCTION__, "category=%d(%f)", idx, out_buffer[idx]);
  int ret = -1;
